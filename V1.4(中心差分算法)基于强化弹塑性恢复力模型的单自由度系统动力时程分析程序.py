...
#基于双折线线性强化恢复力模型和中心差分算法的单自由度系统动力时程分析
...
def u_dsd0(ui_1,ui,u_srd0):#u_dsd0为斜率为k1的曲线上，力开始下降对应的位移。必须大于或等于初始屈服位移u_srd0。
    Δu=ui-ui_1
    if Δu<0 and ui_1>u_srd0:
        return ui_1
    elif Δu<0 and ui_1<=u_srd0:
        return u_srd0
    elif Δu>0 and ui_1<-u_srd0:
        return ui_1
    else:
        return -u_srd0
def F_srd(u_srd0,u_dsd0,F_srd0,k1):#输出后继屈服力。
    if u_dsd0>0:
        return k1*(u_dsd0-u_srd0)+F_srd0#正值
    else:
        return k1*(u_dsd0+u_srd0)-F_srd0#负值
def ReForce(fsi_1,Δu,F_srd,k0,k1):#定义理想弹塑性恢复力-位移规则。fs(i-1) 为前一步的恢复力，Δu为步长的位移差，k为初始刚度。
    if abs(fsi_1)<=F_srd:#F_srd为屈服力(kN)，是一个变量，根据当前的塑性位移而定。如果前一个恢复力小于屈服力，
        if abs(fsi_1+k0*Δu)<=F_srd:#并且当前按照线性计算出的恢复力也小于屈服力(因为按照线性计算出的恢复力要大一点)
            return fsi_1+k0*Δu#那么当前的恢复力等于上一个恢复力按刚度k0线性变化。
        elif 0<fsi_1+k0*Δu:#如果当前的恢复力超过屈服力，而且为正，
            return fsi_1+k1*Δu #那么当前的恢复力按照k1斜率前进。正值
        else:
            return fsi_1+k1*Δu #否则按照k1斜率前进。负值
    elif fsi_1>0:#如果前一个恢复力超过屈服力，而且为正，
        if Δu>0:#并且还在继续屈服，
            return fsi_1+k1*Δu#那么当前的恢复力按照k1斜率前进。正值
        else:
            return fsi_1+k0*Δu#否则就是Δu<0, 按照刚度k0线性返回。正值
    else:#如果前一个恢复力超过屈服力，而且为负，
        if Δu<0:#并且还在继续屈服，
            return fsi_1+k1*Δu#那么当前的恢复力按照k1斜率线性前进。负值
        else:
            return fsi_1+k0*Δu#否则就是Δu>0, 按照刚度k0线性返回。负值     
...
################################      定义初始数据    ##################################
k0=875.5#初始刚度系数
k1=k0/10#弹塑性切线刚度。进入弹塑性阶段时，曲线的斜率。一般取初始刚度的10%
m=17.5#质量
c=35#粘滞阻尼系数
u0=0#初始位移
v0=0#初始速度
...
u_srd0=F_srd0/k0#初始屈服位移

if abs(u0*k0)<F_srd0:#恢复力初值和初始位移有关。
    fs=u0*k0
elif u0>0:
    fs=F_srd0+(u0-u_srd0)*k1
else:
    fs=-F_srd0+(u0+u_srd0)*k1
fs_ls=[]#用于存储恢复力之列表
fs_ls.append(fs)
...
u_ls=[]#定义位移列表，存储位移反应。
u_ls.append(u0-Δt*v0+pow(Δt,2)/2*(P(0)-c*v0-k0*u0)/m)#位移初始条件u,-1。
...
while(round(t,N_Decimal)<T):#当t=Ts时停止计算。完成本时间点的反应计算，再增加Δt，并对计数位移反应的个数i。
    ...
    u_dsd0_value=u_dsd0(u_ls[i-1],u_ls[i-2],u_srd0)#后继屈服位移
    F_srd_value=F_srd(u_srd0,u_dsd0_value,F_srd0,k1)#后继屈服力
    fs=ReForce(fs,u_ls[i-1]-u_ls[i-2],abs(F_srd_value),k0,k1)#第i个恢复力
    fs_ls.append(fs)
...
u_t_Datamngr.Savedata(t_ls,u_ls,t_label,u_label,file_name_u_t)
